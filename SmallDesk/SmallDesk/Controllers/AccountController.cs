...
namespace SmallDesk.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            get { return _database ?? HttpContext.GetOwinContext().Get<ApplicationDbContext>(); }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var role = Database.Roles.Find(model.RoleId);

                if (role == null)
                {
                    ModelState.AddModelError("", "Regra inválida");
                    return View(model);
                }
